...
<!-- SPECKIT START -->
**Active Spec Kit plan**: [`specs/002-spectre-full-coverage/plan.md`](specs/002-spectre-full-coverage/plan.md)
— **Phase: planning. Branch `002-spectre-full-coverage`.**
Extend `Fugue.Adapters.Console` from Phase 1 MVP to **complete coverage** of
the public `Spectre.Console 0.49.*` API. Five user-story PRs in order: P1
custom-renderable bridge → P2 data-display primitives (Tree/Json/BarChart/
BreakdownChart/Calendar/FigletText/Grid/TextPath) → P3 live/streaming
(Live/Status/Progress/Spinners) → P4 interactive prompts (Text/Selection/
MultiSelection/Confirmation) → P5 console infrastructure (AnsiConsole/
TestConsole/exception rendering). Same Phase 1 principles: typed
primitives, `Result`-returning smart constructors, escape-tracked
`SafeText`, no exposed C# types in `.fsi`. After Phase 2: Phase 3
high-level Layout framework for Fugue UX.
Companion artifacts: [spec.md](specs/002-spectre-full-coverage/spec.md),
[checklists/requirements.md](specs/002-spectre-full-coverage/checklists/requirements.md).
Phase 0/1 artifacts (research.md, data-model.md, coverage-matrix.md,
contracts/, quickstart.md) being generated by subagents.
**Phase 1 status: COMPLETE** — merged in PR #942, follow-ups #944 (CI
runner) and #946 (Windows cross-platform) also merged. 656 tests green
on all 3 OSes.
