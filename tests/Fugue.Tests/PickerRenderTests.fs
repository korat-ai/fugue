...
let private run (state: PickerState) : MockTerminal =
    let lineCount = 1 + 1 + 1 + state.VisibleRows + 1 + 1  // title+query+above+items+below+footer
...
///   0 : title
///   1 : search/filter box
///   2 : above-hint or blank
///   3..3+VisibleRows-1 : item rows
///   3+VisibleRows : below-hint or blank
///   4+VisibleRows : footer
let private titleRow   = 0
let private hintAbove  = 2
let private itemRow (offset: int) = 3 + offset
let private hintBelow (visibleRows: int) = 3 + visibleRows
let private footerRow (visibleRows: int) = 4 + visibleRows
...
let ``empty items: renderState produces ops without crashing`` () =
    let state =
        { Title = "Choose"; Query = ""; Items =[||]; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 3 }
    let ops = renderState state
    // 1 (title) + 1 (query box) + 1 (above-hint blank) + 3 (item rows, all empty) + 1 (below-hint blank) + 1 (footer) = 8
    ops |> List.length |> should equal 8

[<Fact>]
let ``empty items: title row contains title text`` () =
    let state =
        { Title = "My Title"; Query = ""; Items =[||]; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 3 }
    let term = run state
...
let ``single item selected: row contains selection marker`` () =
    let state =
        { Title = "Pick"; Query = ""; Items =[| "alpha", "hint-a" |]; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 5 }
    let term = run state
...
let ``three items currentIdx=1: middle row contains marker, first row does not`` () =
    ...
    let state = { Title = "T"; Query = ""; Items =items; CurrentIdx = 1; ScrollTop = 0; VisibleRows = 5 }
    let term = run state
...
let ``scrollTop=2: above-hint row contains 'more above'`` () =
    ...
    let state = { Title = "T"; Query = ""; Items =items; CurrentIdx = 3; ScrollTop = 2; VisibleRows = 5 }
    let term = run state
...
let ``scrollTop=0: above-hint row is blank`` () =
    ...
    let state = { Title = "T"; Query = ""; Items =items; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 5 }
    let term = run state
...
let ``currentIdx outside scroll window: does not throw`` () =
    ...
    let state = { Title = "T"; Query = ""; Items =items; CurrentIdx = 9; ScrollTop = 0; VisibleRows = 3 }
    // If this throws, the test fails naturally — that is the assertion.
    let ops = renderState state
    ops |> List.length |> should equal (state.VisibleRows + 5)
...
let ``item rows include dim markup for hints`` () =
    ...
    let state = { Title = "T"; Query = ""; Items =items; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 3 }
    let ops = renderState state
...
let ``renderState is deterministic`` () =
    let items = Array.init 5 (fun i -> $"item{i}", $"h{i}")
    let state = { Title = "Determinism"; Query = ""; Items =items; CurrentIdx = 2; ScrollTop = 0; VisibleRows = 5 }
...
let ``title is the first Append op`` () =
    let state =
        { Title = "First Op Title"; Query = ""; Items =[| "x","y" |]; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 3 }
    let ops = renderState state
    match ops with
...
let ``footer row contains navigation hint text`` () =
    let state =
        { Title = "T"; Query = ""; Items =[| "a","b" |]; CurrentIdx = 0; ScrollTop = 0; VisibleRows = 3 }
    let term = run state
