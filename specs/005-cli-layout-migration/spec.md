...
- **SC-008**: The eight migrated files contain only adapter API terminology in their import declarations and rendering call sites — verified mechanically by a standing invariant test that asserts zero direct underlying-library symbols are referenced in any of the eight files (this is the auditable proxy for "a new contributor can describe behaviour using only adapter terminology"; the human-judgement form of the check is intentionally not in scope because it cannot be automated). A documentation pointer (comment block in the canonical example file) directs future contributors to the production pattern.
