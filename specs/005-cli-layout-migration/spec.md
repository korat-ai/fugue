...
2. **Given** a user watches the assistant stream a markdown response, **When** the response completes, **Then** the per-frame median render time stays at or below 10 ms and the token-arrival-to-on-screen p95 latency stays at or below 50 ms (per FR-004), with no extra full-frame redraws between tokens.
...
1. **Given** the migration has been on `main` for ≥ 14 calendar days with zero rendering-regression issues filed and the maintainer has confirmed in writing (per FR-007), **When** the maintainer searches the REPL source for the legacy-render fallback environment variable, **Then** zero matches are returned.
...
- **Streaming markdown response arrives in tiny incremental token bursts**: the migration must not introduce flicker or full-frame redraws between tokens, and must hold the FR-004 (b) bound of ≤ 50 ms p95 token-arrival-to-on-screen latency. The rendered text must accumulate as smoothly as it did pre-migration.
...
- **FR-002**: A text-search for the underlying library's namespace across the REPL source tree MUST return zero matches, with the documented exception of two designated bridge files that legitimately import the adapter: `src/Fugue.Cli/LayoutHost.fs` (created in the prior phase) and the new `src/Fugue.Cli/RenderBridge.fs` (created by this feature).
...
- **FR-004**: Existing rendering performance characteristics MUST NOT regress, with two concrete bounds: (a) median per-frame render time on a typical layout tree MUST be at or below **10 ms** (the pre-migration baseline established by the prior phase's performance benchmark) — verified by running the existing performance-traited benchmark suite against post-migration code; (b) end-to-end latency from token-arrival to on-screen render during a streaming response MUST be at or below **50 ms** at the 95th percentile (the human perceptual threshold for streaming text smoothness, ≈ 20 Hz). Long-session memory growth attributable to retained rendering state MUST be non-existent (no monotonic growth visible across a 30-minute synthetic session).
...
- **FR-007**: A legacy-render fallback environment variable from the prior phase MUST be removed from the REPL source tree after **all** of the following soak conditions hold simultaneously: (a) at least **14 calendar days** have elapsed since the last migration pull request (PR P4) merged to `main`; (b) **zero** rendering-regression issues have been filed against the post-migration binary during that window; (c) the maintainer has explicitly confirmed the removal in writing (per the project's CEO-escalation rule).
...
- **SC-008**: All REPL render-path files (the eight in the current inventory plus any added in future) contain only adapter API terminology in their import declarations and rendering call sites — verified mechanically by a standing invariant test that scans every `.fs` file under the REPL source tree (except the two designated bridge files) and asserts zero direct underlying-library symbols are referenced. This is the auditable proxy for "a new contributor can describe behaviour using only adapter terminology"; the human-judgement form of the check is intentionally not in scope because it cannot be automated. A documentation pointer (comment block in the canonical example file) directs future contributors to the production pattern.
...
- **Soak period for fallback removal**: The soak period is concretely **14 calendar days** on `main` with zero rendering-regression issues filed (per FR-007). This rule was adopted in place of the more natural "one full Fugue release cycle" because Fugue ships releases ad-hoc rather than on a fixed cadence, making "release cycle" unmeasurable as a gate. 14 days is long enough to see at least one realistic interactive-use pattern (two weekends of REPL sessions) against the post-migration binary; short enough that the fallback's maintenance cost is not indefinite.
