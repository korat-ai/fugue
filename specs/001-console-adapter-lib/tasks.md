...
- [X] TODO(review-#8): Seal the public `.fsi` signature file surface. **DONE in this PR.** Per-module `.fsi` files were autogenerated via `just regen-fsi` (a new Justfile recipe wrapping `fsc --sig` through MSBuild `OtherFlags` injection) and split per file. The generated baselines already preserved `private` markers on record fields (`Style`, `RenderContext`) and DU constructors (`SafeText`). `Renderer.fsi` was further narrowed by hand to expose only `toRawAnsi` + `toDrawOp` — all Spectre type aliases, `traverseM`, `renderPrimitive`, and `primitiveToRenderable` are now sealed inside the module. Build + 83/83 tests stay green.
- [ ] TODO(review-#8 followup): The historical contract sketch `contracts/Fugue.Adapters.Console.fsi` is now superseded by the real `src/Fugue.Adapters.Console/*.fsi` files. Either delete the sketch or downgrade it to a doc reference. Trivial cleanup; can be folded into the next adapter PR.
